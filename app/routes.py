...
def follow(username):
    ...
    if form.validate_on_submit():
        ...
        if user == current_user:
            flash('You really want to follow yourself?')
...
def unfollow(username):
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            return redirect(url_for('index'))
        if user == current_user:
            flash('You really want to unfollow yourself?')
            return redirect(url_for('user', username=username))
        current_user.unfollow(user)
        db.session.commit()
        flash(f'You are now Unfollowing {username}')
        return redirect(url_for('user',username=username))
    else:
        return redirect(url_for('index'))
...
def explore():
    page = request.args.get('page', 1, type=int)
    posts = Post.query.order_by(Post.timestamp.desc()).paginate(page, app.config['POSTS_PER_PAGE'], False)# Reddit All tab
